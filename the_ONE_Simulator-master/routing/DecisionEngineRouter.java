...
//    protected List<Message> trustToken;
     ...
     */
    protected Map<DTNHost, Set<Message>> trustToken;
    ...
    public DecisionEngineRouter(Settings s) {
        ...
        trustToken = new HashMap<DTNHost, Set<Message>>();
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            if (isObserver(getHost()) && (isVolunteer(otherNode) || isMisbehave(otherNode))) {
                 ...
                 */
                Map<DTNHost, Set<Message>> otherTrustToken = otherRouter.getTrustToken();
                
                //jika verificatingnya tidak kosong dia ngecek verificatingnya udah sesuai jumlah verificatornya belum
                if (!Incentive.getVerificating().isEmpty()) {
                    Incentive.createIncentive();
                }
                //jiks trustoken volunter tidak ksong maka
                if (!otherTrustToken.isEmpty()) {
                    //meminta trusttoken volunteer yang ditemui, dan dicatat ke cloud
                    if(isVolunteer(otherNode)){
                        for (Map.Entry<DTNHost, Set<Message>> entry : otherTrustToken.entrySet()) {
                            DTNHost key = entry.getKey();
                            Set<Message> value = entry.getValue();
                            Incentive.setTrustToken(key, value, true, getHost()); //gethost pengirimnya (sek dititipi)
                        }
                    }
                    ...
                    //meminta trusttoken volunteer yang ditemui, dan dicatat ke cloud
                        for (Map.Entry<DTNHost, Set<Message>> entry : otherTrustToken.entrySet()) {
                            DTNHost key = entry.getKey();
                            Set<Message> value = entry.getValue();
                            Incentive.setTrustToken(key, value, chance, getHost()); //gethost pengirimnya (sek dititipi)
                        }
                    }
...
//                        System.out.println("signature : " + this.getHost());
                    ...
                    if (isMisbehave(getHost())) {
                         ...
                         */
                        List<DTNHost> accomplices = SimScenario.getInstance().getMisbehaves();
                        Set<DTNHost> skip = new HashSet<DTNHost>();
                        ...
                        if((accomplices.size()-1) > skip.size()){
                        
                            do {
                                accomplice = accomplices.get(rand.nextInt(SimScenario.getInstance().getMisbehaves().size()));
                            } while (accomplice.equals(getHost()) || m.getHops().contains(accomplice) || blacklist.contains(accomplice));
...
//        return trustToken;
//    }
    public Map<DTNHost, Set<Message>> getTrustToken() {
        return trustToken;
    }
